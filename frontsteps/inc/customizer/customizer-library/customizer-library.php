...
if (!class_exists('Customizer_Library')) :
    ...
    class Customizer_Library
    {
        ...
        {
            //The following displays repeated options than can be deleted from child theme (as long as values / defaults are the same)
//            if($this->options)debug(array_keys(array_intersect_key($this->options, $options)));

            $this->options = array_replace_recursive($options,$this->options);

//            if(!empty($this->options))debug($this->options);
        }
